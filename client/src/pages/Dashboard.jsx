  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-slate-800">Mes Posts RSS</h2>
          <p className="text-slate-500">{posts.length} post{posts.length > 1 ? 's' : ''} au total</p>
        </div>
      ...
      {filteredPosts.length === 0 ? (
        <div className="text-center py-16 bg-white rounded-2xl border border-slate-200">
          ...
          <h3 className="text-xl font-semibold text-slate-600 mb-2">
            {posts.length === 0 ? 'Aucun post pour le moment' : 'Aucun résultat'}
          </h3>
          <p className="text-slate-400 mb-6">
            {posts.length === 0
              ? 'Commencez par créer votre premier post RSS'
